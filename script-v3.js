...
async function loadDashboard(){const status=$('statusText');$('statusDot').parentElement.classList.remove('connected');status.textContent='Connecting to BNB Chain';$('refresh').disabled=true;try{const r=await fetch('/api/donations');if(!r.ok)throw new Error(`API ${r.status}`);const d=await r.json();if(!d.ok)throw new Error(d.error||'Unavailable');$('totalDelivered').textContent=fmtBNB(d.totalDeliveredBNB);$('pending').textContent=fmtBNB(d.pendingBNB);$('transferCount').textContent=Number(d.transferCount||0).toLocaleString();$('deliveredNote').textContent=d.historyAvailable?'NodeReal verified processor distributions':'Live balance only';$('latestTransfer').textContent=d.latestTimestamp?`Latest ${new Date(d.latestTimestamp*1000).toLocaleString()}`:(d.historyError?'History temporarily unavailable':'No completed distribution returned');status.textContent='Live NodeReal BNB data';$('statusDot').parentElement.classList.add('connected');const txs=d.transactions||[];$('transactions').innerHTML=txs.length?txs.slice(0,5).map(t=>`<div class="tx"><strong>${fmtBNB(t.valueBNB)}</strong><time>${new Date(t.timestamp*1000).toLocaleString()}</time><a href="https://bscscan.com/tx/${t.hash}" target="_blank" rel="noopener">Proof ↗</a></div>`).join(''):`<p class="empty">${d.historyError?'NodeReal connected for live data, but transfer history returned: '+d.historyError:'No completed processor distribution has been returned yet. Flap remains the independent verification source.'}</p>`}catch(e){status.textContent='Dashboard connection unavailable';$('transactions').innerHTML='<p class="empty">The NodeReal endpoint did not respond. Refresh shortly or verify the deployment environment.</p>';console.warn(e)}finally{$('refresh').disabled=false}}
